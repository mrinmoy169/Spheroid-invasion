...
class NewSimulationSteppable(SteppableBasePy):
    ...
    def __init__(self,frequency=1):
        ...
        self.force_mod = -600.01 #active motility force (value of mu)

    def start(self):
        #creating the initial setup for splenocytes
        for ix in np.arange(0,self.dim.x,15):
            for iy in np.arange(0,self.dim.y,15):
                for iz in np.arange(0,self.dim.z,15):
                    if np.sqrt((ix-self.dim.x/2)**2+(iy-self.dim.y/2)**2+(iz-self.dim.z/2)**2)>55:
                        self.cell_field[int(ix):int(ix)+5, int(iy):int(iy)+5, int(iz):int(iz)+5] = self.new_cell(self.T)
        ...
        for cell in self.cell_list:
            ...
            if cell.type == self.T:
                ...
                cell.lambdaVecX= self.force_mod*np.sin(theta)*np.cos(phi)
                cell.lambdaVecY= self.force_mod*np.sin(theta)*np.sin(phi)
                cell.lambdaVecZ= self.force_mod*np.cos(theta)

            cell.dict["postauX"]=cell.xCOM
            cell.dict["postauY"]=cell.yCOM
            cell.dict["postauZ"]=cell.zCOM
            cell.dict["vx"]=0.0
            cell.dict["vy"]=0.0
            cell.dict["vz"]=0.0
    ...
    def step(self,mcs):  
        for cell in self.cell_list_by_type(self.T):
            if mcs%10==0: #10 is the persistence time of cell polarity (tau)
                ...
                dz=(cell.zCOM - cell.dict["postauZ"])
                #pbc correction
                ...
                cell.dict["vy"]=vy
                cell.dict["vz"]=vz
                norm_v = np.linalg.norm([vx,vy,vz])
                if norm_v != 0:
                    vx /= norm_v
                    vy /= norm_v
                    vz /= norm_v
                ...
                cell.lambdaVecZ = self.force_mod*vz
        ...
        for cell in self.cell_list_by_type(self.T):
            ...
            if area > 0.95*cell.surface: #0.95 is the contact dependent growth parameter f
                cell.targetVolume += 125/10 #20000 is the doubling time of splenocytes
                cell.targetSurface = 7.5*(cell.targetVolume)**(2/3)
...
#calculating the splenocite positions
        if mcs%1000==0:
            ...
            list_r1=[]
            for cell in self.cell_list_by_type(self.T):
                list_r.append(np.sqrt((cell.xCOM-self.dim.x/2)**2+(cell.yCOM-self.dim.y/2)**2+(cell.zCOM-self.dim.z/2)**2))
                pixel_list = self.get_cell_pixel_list(cell)
                for pixel_tracker_data in pixel_list:
                    pt=pixel_tracker_data.pixel
                    list_r1.append(np.sqrt((pt.x-self.dim.x/2)**2+(pt.y-self.dim.y/2)**2+(pt.z-self.dim.z/2)**2))
...
#calculating the boundary of the tumor
        ...
        if mcs%1000==0:
            ...
            if output_dir is not None:
                output_path = Path(output_dir).joinpath('boundary_' + str(mcs) + '.dat')
                with open(output_path, 'w') as fout:
                    for item in list_E:
                        fout.write('{} {} {}\n'.format(item.xCOM, item.yCOM, item.zCOM))
        ...
        if mcs%1000==0:
            ...
            if output_dir is not None:
                ...
                with open(output_path, 'w') as fout:
                    for cell in self.cell_list:
                        fout.write('{} {} {} {} {} {} {}\n'.format(cell.id, cell.type, cell.xCOM, cell.yCOM, cell.zCOM, cell.volume, cell.surface))
                        
        if mcs%1000==0:
            output_dir = self.output_dir
            if output_dir is not None:
                output_path = Path(output_dir).joinpath('step1_' + str(mcs) + '.dat')
                with open(output_path, 'w') as fout:
                    for cell in self.cell_list_by_type(self.T):
                        pixel_list = self.get_cell_pixel_list(cell)
                        for pixel_tracker_data in pixel_list:
...
class MitosisSteppable(MitosisSteppableBase):
    ...
    def update_attributes(self):
        ...
        self.parent_cell.targetSurface = 7.5*(self.parent_cell.targetVolume)**(2/3)
